...
export async function Experience({ experiences }: ExperienceProps): Promise<JSX.Element> {
	const t = await getTranslations('experience');

	return (
		<section id='experience' aria-label='Professional experience' className='py-24 md:py-32'>
			<div className='mx-auto max-w-6xl px-6'>
				<SectionHeader number='02' label={t('sectionLabel')} heading={t('heading')} />

				<ol className='relative space-y-12'>
					{experiences.map((exp) => {
						const description = t.raw(`${exp.id}.description`) as string[];
						return (
							<li key={exp.id} className='grid gap-4 sm:grid-cols-[200px_1fr]'>
								<div className='font-body text-foreground/40 pt-1 text-sm'>
									...
									{exp.current ? <span className='text-accent font-medium'>{t('present')}</span> : <time dateTime={exp.endDate}>{formatDate(exp.endDate ?? '')}</time>}
									<p className='text-foreground/30 mt-1 text-xs'>{getDuration(exp.startDate, exp.endDate)}</p>
								</div>

								<div>
									<h3 className='font-display text-xl font-semibold'>{t(`${exp.id}.role`)}</h3>
